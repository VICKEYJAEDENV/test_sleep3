...
document.getElementById('sleepButton').addEventListener('click', function() {
    ...
    if (this.innerText === 'Start') {
        ...
        outputDiv.innerHTML = `
            ...
            <div class="neon-box">${formatDate(startTime)} <span style="font-size: 28px;">${formatTime(startTime)}</span></div>
        `;
        this.innerText = 'End';
        ...
        outputDiv.innerHTML = `
            ...
            <div class="neon-box">${formatDate(endTime)} <span style="font-size: 28px;">${formatTime(endTime)}</span></div>
            <div class="centered-text">Total Sleep</div>
            <div class="neon-box">${hours}H : ${minutes}M</div>
        `;
